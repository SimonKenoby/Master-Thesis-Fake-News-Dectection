...
python attention_lstm.py $train $out_dir/$out_prefix $word2vec $dictFile $SEQ_LENGTH $EMBEDDING_DIM $HIDDEN $LAYERS $DROPOUT $BATCH_SIZE $EPOCHS $logs --test $test --experiment $experiment --utils $utils --db TFE --collection test --host localhost --port 27017
